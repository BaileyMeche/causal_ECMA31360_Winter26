...
Substitution bias (scenario): Control customers (not o!ered membership) respond by substituting to other actions/programs that a!ect Walmart.com spend, changing the control outcome relative to what it would have been absent
the experiment. Examples: controls switch spending to in-store purchases to avoid shipping costs; controls enroll in a different paid program (or use a partner’s account) that changes their Walmart.com spend; controls change shopping behavior
because they are denied the o!er.

Contamination bias (scenario): The control group is partially “treated” (or treated group not cleanly treated), so
outcomes for control reflect exposure to treatment benefits. Examples: account-sharing of membership benefits with
control customers; operationally extending free shipping rules to nonmembers
---
...
\[\rho = E[\rho_i] \text{the average effect of having a membership across all customers} \]
\[\rho_1 = E[\rho_i | D_i= 1 ] \text{the average effect of having a membership among members} \]
\[\rho_0 = E[\rho_i | D_i= 0 ] \text{the average effect of having a membership among non-members} \]

Assuming $\rho_0=\rho_1$ means members and non-members have the same average gain from membership. Therefore, membership status is unrelated to how the  membership would effect them. Thus membership selection is not based on who benefits more. This is the reason assumption is called “no selection on gains”
---
...
If we are assuming no selection on gains, thus $\rho = \rho_1$. Then identifying $\rho_1$ also identifies $\rho$
---
